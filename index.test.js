...
const PROFILE_INPUTS = { "role-to-assume": ROLE_ARN, "aws-region": FAKE_REGION, "aws-profile": "test-profile" };
...
describe("Configure AWS Credentials", () => {
  ...
  test("web identity token file provided with absolute path", async () => {
    ...
    fs.promises.readFile.mockResolvedValueOnce("testpayload");
  ...
  test("web identity token file does not exist", async () => {
    process.env.SHOW_STACK_TRACE = "false";
    core.getInput = jest.fn().mockImplementation(
      mockGetInput({
        "role-to-assume": ROLE_ARN,
    ...
    await run();
    expect(core.setFailed).toHaveBeenCalledWith("Web identity token file does not exist: /home/github/fake/token/file");
  });
  ...
  describe("proxy settings", () => {
    test("setting proxy with actions input", async () => {
      ...
      core.getInput = jest.fn().mockImplementation(mockGetInput({ ...DEFAULT_INPUTS, "http-proxy": EXPECTED_PROXY }));
  ...
  describe("Profile Support", () => {
    test("use profile and save credentials file", async () => {
      process.env = {
      ...
      core.getInput = jest.fn().mockImplementation(mockGetInput(PROFILE_INPUTS));
      await run();

      // Don't export aws credentials
      expect(core.exportVariable).toHaveBeenCalledTimes(5);
      expect(core.exportVariable).toHaveBeenNthCalledWith(1, "AWS_DEFAULT_REGION", FAKE_REGION);
      expect(core.exportVariable).toHaveBeenNthCalledWith(2, "AWS_REGION", FAKE_REGION);
      expect(core.exportVariable).toHaveBeenNthCalledWith(3, "AWS_PROFILE", PROFILE_INPUTS["aws-profile"]);
      expect(core.exportVariable).toHaveBeenNthCalledWith(4, "AWS_SHARED_CREDENTIALS_FILE", "/home/.aws/credentials");
      expect(core.exportVariable).toHaveBeenNthCalledWith(5, "AWS_CONFIG_FILE", "/home/.aws/config");

      expect(fs.promises.writeFile).toHaveBeenCalledTimes(2);
      expect(fs.promises.writeFile).toHaveBeenNthCalledWith(1, "/home/.aws/config", expect.toMatchSnapshot());
      expect(fs.promises.writeFile).toHaveBeenNthCalledWith(2, "/home/.aws/credentials", expect.toMatchSnapshot());
    });
    ...
    test("add new profile if credentials file already exists", async () => {
      fs.existsSync
      ...
      fs.promises.readFile.mockResolvedValueOnce(`[profile default]
        region=fake-region-1`).mockResolvedValueOnce(`[default]
    ...
    test("web identity token file provided with relative path", async () => {
      ...
      // Identity token file is read from the workspace root
      fs.promises.readFile.mockImplementationOnce(() => Promise.resolve("testpayload"));

      fs.promises.readFile.mockResolvedValueOnce("").mockResolvedValueOnce("");
